...
public class FileController {
    ...
    public ResponseEntity<Map<String, String>> uploadFile(@Valid @ModelAttribute FileUploadRequest request) {
        try {
            ...
            throw new IllegalArgumentException("Invalid meta data: " + e.getMessage(), e);
    ...
    public ResponseEntity<Map<String, Object>> getFileMetadata(@RequestBody Map<String, List<String>> request) {
        try {
            ...
            Map<String, Object> response = metadataMap.entrySet().stream().collect(Collectors.toMap(Map.Entry::getKey, entry -> Map.of("token", entry.getKey(), "filename", entry.getValue().getName(), "size", entry.getValue().getSize(), "contentType", entry.getValue().getContentType(), "createTime", entry.getValue().getCreateTime().toInstant().toString(), "meta", entry.getValue().getMeta())));
    ...
    public ResponseEntity<?> downloadFile(@PathVariable String token) {
        try {
            ...
            return ResponseEntity.ok().header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=\"" + file.getName() + "\"").header("X-Filename", file.getName()).header("X-Filesize", String.valueOf(file.length())).header("X-CreateTime", metadata.getCreateTime().toInstant().toString()).body(resource);
    ...
    @DeleteMapping("/{token}")
    public ResponseEntity<ErrorMessage> deleteFile(@PathVariable UUID token) {
        try {
